...
rtems_task_priority task_prio[ ] = { 2, 4, 6, 8, 10 };
rtems_interval      task_period[ ] = { 40, 40, 80, 100, 120 };
...
)
{
    ...
    status = rtems_semaphore_create(
        rtems_build_name('M', 'U', 'X', '2'),
        1,
        RTEMS_BINARY_SEMAPHORE | RTEMS_PRIORITY | RTEMS_PRIORITY_CEILING,
        task_prio[ 2 ] - 1,
        &mux2
    );
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_semaphore_create2");
    ...
    status = rtems_task_create(
    rtems_build_name( 'T', 'K', 'R', '4' ), 1, RTEMS_MINIMUM_STACK_SIZE, RTEMS_DEFAULT_MODES,
    RTEMS_DEFAULT_ATTRIBUTES, &release_task_id[ 3 ]
    );
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_create_4.1");
    status = rtems_task_create(
    task_name[ 3 ], task_prio[ 3 ], RTEMS_MINIMUM_STACK_SIZE, RTEMS_DEFAULT_MODES,
    RTEMS_DEFAULT_ATTRIBUTES, &task_id[ 3 ]
    );
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_create_4.2");

    status = rtems_task_create(
    rtems_build_name( 'T', 'K', 'R', '5' ), 1, RTEMS_MINIMUM_STACK_SIZE, RTEMS_DEFAULT_MODES,
    RTEMS_DEFAULT_ATTRIBUTES, &release_task_id[ 4 ]
    );
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_create_5.1");
    status = rtems_task_create(
    task_name[ 4 ], task_prio[ 4 ], RTEMS_MINIMUM_STACK_SIZE, RTEMS_DEFAULT_MODES,
    RTEMS_DEFAULT_ATTRIBUTES, &task_id[ 4 ]
    );
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_create_5.2");
    ...
    status = rtems_task_start( release_task_id[ 3 ], Tsk4_release, 4 );
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_start_4.1");
    status = rtems_task_start( release_task_id[ 4 ], Tsk5_release, 5 );
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_start_5.1");

    log_send("START");

    int loops = 3;

    // while (!(task_exited[0] && task_exited[1]) && loops)
    // {
    //     loops--;
    //     rtems_task_wake_after(max_iter*task_period[1]);
    // }
    ...
    {
        if (!task_exited[0])
            printf ("\nerror: tasks1 did not delete\n");
        if (!task_exited[1])
            printf ("\nerror: tasks2 did not delete\n");
        if (!task_exited[2])
            printf ("\nerror: tasks3 did not delete\n");
        if (!task_exited[3])
            printf ("\nerror: tasks4 did not delete\n");
        if (!task_exited[4])
            printf ("\nerror: tasks5 did not delete\n");
        exit( 1 );
    }
...
)
{
    ...
    rtems_task_wake_after(task_period[ 0 ]);
...
)
{
    ...
    rtems_task_wake_after(task_period[ 1 ]);
...
)
{
    ...
    busy_sleep_ms(6);
    log_send( "t9" );
    status = rtems_semaphore_obtain(mux2, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_semaphore_obtain");
    log_send( "t10" );
    busy_sleep_ms(1);
    log_send( "t11" );
    status = rtems_semaphore_release(mux2);
    check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_semaphore_release");
...
)
{
    ...
    rtems_task_wake_after(task_period[ 2 ]);

    uint32_t iter = 0;
    while ( iter < max_iter ) {
        iter ++;
        status = rtems_rate_monotonic_period( period, task_period[ 2 ] );
        if ( status == RTEMS_TIMEOUT )
            break;
        /* Start periodic job */
...
)
{
    ...
    rtems_task_wake_after(task_period[ 3 ]);

    uint32_t iter = 0;
    while ( iter < max_iter ) {
        iter ++;
        status = rtems_rate_monotonic_period( period, task_period[ 3 ] );
        if ( status == RTEMS_TIMEOUT )
            break;
        /* Start periodic job */
        status = rtems_task_is_suspended(task_id[ 3 ]);
        if ( status == RTEMS_ALREADY_SUSPENDED )
        {
            status = rtems_task_restart( task_id[ 3 ], iter );
            check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_restart_4.2");
        }
        else
        {
            status = rtems_task_start( task_id[ 3 ], Tsk4_job, iter );
            check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_start_4.2");
        }
        log_send( "t12" );
    }
    /* missed period so delete period and SELF */
...
)
{
    ...
    while ( iter < max_iter ) {
        ...
        status = rtems_task_is_suspended(task_id[ 4 ]);
        if ( status == RTEMS_ALREADY_SUSPENDED )
        {
            status = rtems_task_restart( task_id[ 4 ], iter );
            check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_restart_5.2");
        }
        else
        {
            status = rtems_task_start( task_id[ 4 ], Tsk5_job, iter );
            check_status_fatal(status,RTEMS_SUCCESSFUL,"rtems_task_start_5.2");
        }
        log_send( "t18" );
    }
